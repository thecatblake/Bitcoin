...
TEST(Transaction, input_parse) {
    ...
    int len;
    TxIn txin = TxIn::parse(txin_raw, &len);
    for (int i=0; i < sizeof(prev_tx); i++) {
        ASSERT_EQ(txin.prev_tx[i], prev_tx[i]);
    }
    EXPECT_EQ(txin.prev_index, 0);
}
...
TEST(Transaction, parse) {
    ...
    auto tx = transaction::parse(tx_raw);
